...
@Component({
    selector: 'jhi-school-mojobs',
    templateUrl: './school-mojobs.component.html'
})
export class SchoolMojobsComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/school-mojobs'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/school-mojobs', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: SchoolMojobs) {
        return item.id;
    }
